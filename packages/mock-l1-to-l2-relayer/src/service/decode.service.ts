...
export const generateProcessDepositsCalldata = async (
  ethereumClient: PublicClient,
  l1SentMessageEvent: L1SentMessageEvent,
) => {
...
const fetchLatestRejectedIds = async (
  ...
  const depositsAnalyedAndRelayedEvents = await fetchEvents<DepositsAnalyzedAndRelayedEvent>(
    ethereumClient,
    {
  ...
  if (depositsAnalyedAndRelayedEvents.length === 0) {
    throw new Error("No DepositsAnalyzedAndRelayed event found");
  }

  if (depositsAnalyedAndRelayedEvents.length > 1) {
    throw new Error("Multiple DepositsAnalyzedAndRelayed events found");
  }

  return depositsAnalyedAndRelayedEvents[0].args.rejectDepositIds;
};
...
const decodeL1SentMessage = (l1SentMessageEvent: L1SentMessageEvent) => {
