...
export const generateProcessCalldata = async (claimableRequest: ScrollMessengerResult) => {
  return config.STAKING_RELAYER_ENABLED
    ? generateProcessStakingCalldata(claimableRequest)
    : generateProcessWithdrawalsCalldata(claimableRequest);
};

export const generateProcessWithdrawalsCalldata = async (
...
const generateProcessStakingCalldata = async (claimableRequest: ScrollMessengerResult) => {
  const { withdrawals, claimables } = decodeL2SentMessage(claimableRequest);
